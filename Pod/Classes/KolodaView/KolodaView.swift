...
public protocol KolodaViewDelegate: class {
    ...
    func koloda(_ koloda: KolodaView, didSelectCardAt index: Int,_ position: CGPoint)
    func kolodaShouldApplyAppearAnimation(_ koloda: KolodaView) -> Bool
    func kolodaShouldMoveBackgroundCard(_ koloda: KolodaView) -> Bool
    func kolodaShouldTransparentizeNextCard(_ koloda: KolodaView) -> Bool
    func koloda(_ koloda: KolodaView, draggedCardWithPercentage finishPercentage: CGFloat, in direction: SwipeResultDirection, with dragDistance: CGPoint)
...
open class KolodaView: UIView, DraggableCardDelegate {
    ...
    open func frameForCard(at index: Int) -> CGRect {
        ...
        let height = index > 0 ? (self.frame.height - bottomOffset - topOfcfset) * pow(scalePercent, CGFloat(index)) : self.frame.height
    ...
    func card(_ card: DraggableCardView, wasDraggedWithFinishPercentage percentage: CGFloat, inDirection direction: SwipeResultDirection, with dragDistance: CGPoint) {
        
        if let shouldMove = delegate?.kolodaShouldMoveBackgroundCard(self), shouldMove {
            self.moveOtherCardsWithPercentage(percentage)
        }
        delegate?.koloda(self, draggedCardWithPercentage: percentage, in: direction, with: dragDistance)
    }
    ...
    func card(cardWasTapped card: DraggableCardView, at position: CGPoint) {
        guard let visibleIndex = visibleCards.firstIndex(of: card) else { return }
        
        let index = currentCardIndex + visibleIndex
        delegate?.koloda(self, didSelectCardAt: index, position)
    }
